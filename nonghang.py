...
# 传入参数添加到字典里面
def bank_add(account, username, password, country, province, street, door,atype):
...
def useradd():
    ...
    door = input("\t\t请输入您的门牌号")
    atype = input("\t\t请输入账户类型（1或2)")
    add = bank_add(account, username, password, country, province, street, door,atype)
    if add == 3:
        print("数据库已满请到其他银行开户")
    elif add == 2:
        print("请输入其他用户名")
        ...
        info = '''
                ...
                '''
        # 每个元素都可传入%
        print(info % (username, account, country, province, street, door, bank[username]["money"],atype, bank_name))
...
def bank_zz():
    ...
    if (username_zr, username_zc) in bank:
        if pwd in bank[username_zc]["password"]:
            if username_zc["atype"]== "1":
                zz_money = input("请输入转账金额：")
                if zz_money <= bank[username_zc]["money"] and zz_money<=50000:
                    ...
                    return 3
            if username_zc["atype"]== "2":
                zz_money = input("请输入转账金额：")
                if zz_money <= bank[username_zc]["money"] and zz_money <= 20000:
                    bank[username_zc]["money"] -= zz_money
                    bank[username_zr]["money"] += zz_money
                    info = '''
                                                                ------------个人信息------------
